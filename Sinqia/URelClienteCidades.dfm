object frmRelClientesCidades: TfrmRelClientesCidades
  Left = 0
  Top = 0
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'TELA DE RELAT'#211'RIO'
  ClientHeight = 480
  ClientWidth = 463
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  PixelsPerInch = 96
  TextHeight = 13
  object btnGerarRelatorio: TBitBtn
    Left = 8
    Top = 425
    Width = 166
    Height = 50
    Caption = 'Gerar Relatorio'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    Glyph.Data = {
      
      DCDCFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
    ParentFont = False
    TabOrder = 0
    OnClick = btnGerarRelatorioClick
  end
  object Panel2: TPanel
    Left = 8
    Top = 132
    Width = 446
    Height = 103
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 1
    object Label3: TLabel
      Left = 32
      Top = 40
      Width = 89
      Height = 16
      Caption = 'C'#243'digo Inicial:'
    end
    object Label8: TLabel
      Left = 32
      Top = 13
      Width = 365
      Height = 16
      Caption = 'Digite os c'#243'digos iniciais e finais das cidades para filtrar:'
    end
    object Label4: TLabel
      Left = 32
      Top = 72
      Width = 80
      Height = 16
      Caption = 'C'#243'digo Final:'
    end
    object edtCodigoCidadeFinal: TEdit
      Left = 158
      Top = 66
      Width = 145
      Height = 24
      TabOrder = 1
      OnKeyPress = edtCodigoCidadeFinalKeyPress
    end
    object edtCodigoCidadeInicial: TEdit
      Left = 158
      Top = 35
      Width = 145
      Height = 24
      TabOrder = 0
      OnKeyPress = edtCodigoCidadeInicialKeyPress
    end
  end
  object Panel3: TPanel
    Left = 8
    Top = 373
    Width = 446
    Height = 46
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 2
    object chkAgruparCidade: TCheckBox
      Left = 16
      Top = 0
      Width = 376
      Height = 46
      Caption = 'Selecione caso queira agrupar os registros por cidade'
      TabOrder = 0
    end
  end
  object Panel4: TPanel
    Left = 8
    Top = 8
    Width = 446
    Height = 118
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 3
    object Label1: TLabel
      Left = 32
      Top = 44
      Width = 89
      Height = 16
      Caption = 'C'#243'digo Inicial:'
    end
    object Label2: TLabel
      Left = 32
      Top = 82
      Width = 80
      Height = 16
      Caption = 'C'#243'digo Final:'
    end
    object Label7: TLabel
      Left = 32
      Top = 9
      Width = 366
      Height = 16
      Caption = 'Digite os c'#243'digos iniciais e finais dos clientes para filtrar:'
    end
    object edtCodigoClienteFinal: TEdit
      Left = 158
      Top = 79
      Width = 145
      Height = 24
      TabOrder = 1
      OnKeyPress = edtCodigoClienteFinalKeyPress
    end
    object edtCodigoClienteInicial: TEdit
      Left = 158
      Top = 41
      Width = 145
      Height = 24
      TabOrder = 0
      OnKeyPress = edtCodigoClienteInicialKeyPress
    end
  end
  object Panel1: TPanel
    Left = 8
    Top = 237
    Width = 446
    Height = 116
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 4
    object Label5: TLabel
      Left = 32
      Top = 53
      Width = 42
      Height = 16
      Caption = 'Inicial:'
    end
    object Label6: TLabel
      Left = 182
      Top = 53
      Width = 33
      Height = 16
      Caption = 'Final:'
    end
    object Label9: TLabel
      Left = 32
      Top = 21
      Width = 360
      Height = 16
      Caption = 'Selecione o intervalo por qual deseja filtrar os estados:'
    end
    object edtEstadoInicial: TEdit
      Left = 32
      Top = 83
      Width = 121
      Height = 24
      MaxLength = 2
      TabOrder = 0
    end
    object edtEstadoFinal: TEdit
      Left = 182
      Top = 83
      Width = 121
      Height = 24
      MaxLength = 2
      TabOrder = 1
    end
  end
  object frxDBDataset1: TfrxDBDataset
    UserName = 'RelClientesCidades'
    CloseDataSource = False
    DataSource = dsrelClientesCidades
    BCDToCurrency = False
    Left = 112
    Top = 528
  end
  object frxReport1: TfrxReport
    Version = '5.1.5'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Default'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 45690.946174120400000000
    ReportOptions.LastChange = 45690.946174120400000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'begin'
      ''
      'end.')
    Left = 192
    Top = 528
    Datasets = <
      item
        DataSet = frxDBDataset1
        DataSetName = 'RelClientesCidades'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      PaperWidth = 210.000000000000000000
      PaperHeight = 297.000000000000000000
      PaperSize = 9
      LeftMargin = 10.000000000000000000
      RightMargin = 10.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      object PageHeader1: TfrxPageHeader
        FillType = ftBrush
        Height = 120.944960000000000000
        Top = 18.897650000000000000
        Width = 718.110700000000000000
        object Memo11: TfrxMemoView
          Left = 200.315090000000000000
          Top = 15.118120000000000000
          Width = 336.378170000000000000
          Height = 37.795300000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -24
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'RELAT'#211'RIO DE CLIENTES')
          ParentFont = False
        end
        object Line1: TfrxLineView
          Align = baWidth
          Top = 83.149660000000000000
          Width = 718.110700000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
      end
      object MasterData1: TfrxMasterData
        FillType = ftBrush
        Height = 170.078850000000000000
        Top = 200.315090000000000000
        Width = 718.110700000000000000
        DataSet = frxDBDataset1
        DataSetName = 'RelClientesCidades'
        RowCount = 0
        object Shape3: TfrxShapeView
          Top = 68.031540000000000000
          Width = 718.110700000000000000
          Height = 18.897650000000000000
          Fill.BackColor = clScrollBar
        end
        object Shape2: TfrxShapeView
          Top = 3.779530000000000000
          Width = 718.110700000000000000
          Height = 18.897637795275600000
          Fill.BackColor = clScrollBar
        end
        object RelClientesCidadesNome_Cliente: TfrxMemoView
          Left = 37.795300000000000000
          Top = 34.015770000000000000
          Width = 207.874150000000000000
          Height = 18.897650000000000000
          DataField = 'Nome_Cliente'
          DataSet = frxDBDataset1
          DataSetName = 'RelClientesCidades'
          Memo.UTF8W = (
            '[RelClientesCidades."Nome_Cliente"]')
        end
        object Memo2: TfrxMemoView
          Left = 37.795300000000000000
          Top = 3.779530000000000000
          Width = 207.874150000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Nome do Cliente')
          ParentFont = False
        end
        object RelClientesCidadesNome_Cidade: TfrxMemoView
          Left = 37.795300000000000000
          Top = 98.267780000000000000
          Width = 113.385900000000000000
          Height = 18.897650000000000000
          DataField = 'Nome_Cidade'
          DataSet = frxDBDataset1
          DataSetName = 'RelClientesCidades'
          Memo.UTF8W = (
            '[RelClientesCidades."Nome_Cidade"]')
        end
        object RelClientesCidadesEstado_UF_Cidade: TfrxMemoView
          Left = 257.008040000000000000
          Top = 98.267780000000000000
          Width = 188.976500000000000000
          Height = 18.897650000000000000
          DataField = 'Bairro_Cliente'
          DataSet = frxDBDataset1
          DataSetName = 'RelClientesCidades'
          Memo.UTF8W = (
            '[RelClientesCidades."Bairro_Cliente"]')
        end
        object Memo4: TfrxMemoView
          Left = 37.795300000000000000
          Top = 68.031540000000000000
          Width = 94.488250000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Cidade')
          ParentFont = False
        end
        object Memo5: TfrxMemoView
          Left = 257.008040000000000000
          Top = 68.031540000000000000
          Width = 45.354360000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Bairro')
          ParentFont = False
        end
        object RelClientesCidadesCGC_CPF_Cliente: TfrxMemoView
          Left = 294.803340000000000000
          Top = 34.015770000000000000
          Width = 196.535560000000000000
          Height = 18.897650000000000000
          DataField = 'CGC_CPF_Cliente'
          DataSet = frxDBDataset1
          DataSetName = 'RelClientesCidades'
          Memo.UTF8W = (
            '[RelClientesCidades."CGC_CPF_Cliente"]')
        end
        object Memo6: TfrxMemoView
          Left = 294.803340000000000000
          Top = 3.779530000000000000
          Width = 94.488250000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'CPF   |   CNPJ')
          ParentFont = False
        end
        object RelClientesCidadesTelefone_Cliente: TfrxMemoView
          Left = 529.134200000000000000
          Top = 34.015770000000000000
          Width = 151.181200000000000000
          Height = 18.897650000000000000
          DataField = 'Telefone_Cliente'
          DataSet = frxDBDataset1
          DataSetName = 'RelClientesCidades'
          Memo.UTF8W = (
            '[RelClientesCidades."Telefone_Cliente"]')
        end
        object Memo7: TfrxMemoView
          Left = 529.134200000000000000
          Top = 3.779530000000000000
          Width = 151.181200000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Telefone')
          ParentFont = False
        end
        object RelClientesCidadesEstado_UF_Cidade1: TfrxMemoView
          Left = 181.417440000000000000
          Top = 98.267780000000000000
          Width = 41.574830000000000000
          Height = 18.897650000000000000
          DataField = 'Estado_UF_Cidade'
          DataSet = frxDBDataset1
          DataSetName = 'RelClientesCidades'
          Memo.UTF8W = (
            '[RelClientesCidades."Estado_UF_Cidade"]')
        end
        object Memo8: TfrxMemoView
          Left = 181.417440000000000000
          Top = 68.031540000000000000
          Width = 41.574830000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'UF')
          ParentFont = False
        end
        object RelClientesCidadesCEP_Cliente: TfrxMemoView
          Left = 415.748031496063000000
          Top = 98.267780000000000000
          Width = 98.267716535433080000
          Height = 18.897650000000000000
          DataField = 'CEP_Cliente'
          DataSet = frxDBDataset1
          DataSetName = 'RelClientesCidades'
          Memo.UTF8W = (
            '[RelClientesCidades."CEP_Cliente"]')
        end
        object RelClientesCidadesEndereco_Cliente: TfrxMemoView
          Left = 529.134200000000000000
          Top = 98.267780000000000000
          Width = 151.181200000000000000
          Height = 18.897650000000000000
          DataField = 'Endereco_Cliente'
          DataSet = frxDBDataset1
          DataSetName = 'RelClientesCidades'
          Memo.UTF8W = (
            '[RelClientesCidades."Endereco_Cliente"]')
        end
        object Memo9: TfrxMemoView
          Left = 415.748031496063000000
          Top = 68.031540000000000000
          Width = 64.252010000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'CEP')
          ParentFont = False
        end
        object Memo10: TfrxMemoView
          Left = 529.134200000000000000
          Top = 68.031540000000000000
          Width = 151.181200000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Endere'#231'o')
          ParentFont = False
        end
        object Shape1: TfrxShapeView
          Top = 3.779530000000000000
          Width = 718.110700000000000000
          Height = 128.504020000000000000
        end
      end
      object PageFooter1: TfrxPageFooter
        FillType = ftBrush
        Height = 30.236240000000000000
        Top = 430.866420000000000000
        Width = 718.110700000000000000
        object SysMemo1: TfrxSysMemoView
          Left = 154.960730000000000000
          Top = 3.779530000000000000
          Width = 71.811070000000000000
          Height = 18.897650000000000000
          Memo.UTF8W = (
            '[COUNT(MasterData1,2)]')
        end
        object Memo12: TfrxMemoView
          Left = 37.795300000000000000
          Top = 3.779530000000000000
          Width = 109.606370000000000000
          Height = 18.897650000000000000
          Memo.UTF8W = (
            'Total de Clientes:')
        end
        object SysMemo2: TfrxSysMemoView
          Left = 582.047620000000000000
          Top = 3.779530000000000000
          Width = 94.488250000000000000
          Height = 18.897650000000000000
          Memo.UTF8W = (
            '[DATE]')
        end
      end
    end
  end
  object dsrelClientesCidades: TDataSource
    AutoEdit = False
    DataSet = DM.qryRelClientesCidades
    Left = 48
    Top = 528
  end
  object frxRelAgrupado: TfrxReport
    Version = '5.1.5'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Default'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 45690.946174120400000000
    ReportOptions.LastChange = 45690.946174120400000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'begin'
      ''
      'end.')
    Left = 248
    Top = 528
    Datasets = <
      item
        DataSet = frxDBDataset1
        DataSetName = 'RelClientesCidades'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      PaperWidth = 210.000000000000000000
      PaperHeight = 297.000000000000000000
      PaperSize = 9
      LeftMargin = 10.000000000000000000
      RightMargin = 10.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      object PageHeader1: TfrxPageHeader
        FillType = ftBrush
        Height = 90.708720000000000000
        Top = 18.897650000000000000
        Width = 718.110700000000000000
        object Memo14: TfrxMemoView
          Left = 139.842610000000000000
          Top = 3.779530000000000000
          Width = 476.220780000000000000
          Height = 41.574830000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -24
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'RELAT'#211'RIO DE CLIENTES POR CIDADE')
          ParentFont = False
        end
        object Line1: TfrxLineView
          Align = baWidth
          Top = 83.149660000000000000
          Width = 718.110700000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
      end
      object PageFooter1: TfrxPageFooter
        FillType = ftBrush
        Height = 30.236240000000000000
        Top = 449.764070000000000000
        Width = 718.110700000000000000
        object SysMemo1: TfrxSysMemoView
          Left = 154.960730000000000000
          Top = 3.779530000000000000
          Width = 109.606370000000000000
          Height = 18.897650000000000000
          Memo.UTF8W = (
            '[COUNT(MasterData1,2)]')
        end
        object Memo15: TfrxMemoView
          Left = 37.795300000000000000
          Top = 3.779530000000000000
          Width = 120.944960000000000000
          Height = 18.897650000000000000
          Memo.UTF8W = (
            'Total de Cidades:')
        end
        object SysMemo2: TfrxSysMemoView
          Left = 514.016080000000000000
          Top = 3.779530000000000000
          Width = 166.299320000000000000
          Height = 18.897650000000000000
          Memo.UTF8W = (
            '[DATE]')
        end
      end
      object MasterData1: TfrxMasterData
        FillType = ftBrush
        Height = 128.504020000000000000
        Top = 260.787570000000000000
        Width = 718.110700000000000000
        DataSet = frxDBDataset1
        DataSetName = 'RelClientesCidades'
        RowCount = 0
        object Shape3: TfrxShapeView
          Top = 64.252010000000000000
          Width = 718.110700000000000000
          Height = 22.677165354330700000
          Fill.BackColor = clScrollBar
        end
        object Shape2: TfrxShapeView
          Top = 11.338590000000000000
          Width = 718.110700000000000000
          Height = 22.677165354330700000
          Fill.BackColor = clScrollBar
        end
        object Memo3: TfrxMemoView
          Left = 37.795300000000000000
          Top = 37.795300000000000000
          Width = 302.362400000000000000
          Height = 18.897650000000000000
          DataField = 'Nome_Cliente'
          DataSet = frxDBDataset1
          DataSetName = 'RelClientesCidades'
          Memo.UTF8W = (
            '[RelClientesCidades."Nome_Cliente"]')
        end
        object Memo4: TfrxMemoView
          Left = 37.795300000000000000
          Top = 15.118120000000000000
          Width = 147.401670000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Cliente')
          ParentFont = False
        end
        object Memo17: TfrxMemoView
          Left = 37.795300000000000000
          Top = 90.708720000000000000
          Width = 94.488250000000000000
          Height = 18.897650000000000000
          DataField = 'Nome_Cidade'
          DataSet = frxDBDataset1
          DataSetName = 'RelClientesCidades'
          Memo.UTF8W = (
            '[RelClientesCidades."Nome_Cidade"]')
        end
        object Memo18: TfrxMemoView
          Left = 173.858380000000000000
          Top = 90.708720000000000000
          Width = 45.354360000000000000
          Height = 18.897650000000000000
          DataField = 'Estado_UF_Cidade'
          DataSet = frxDBDataset1
          DataSetName = 'RelClientesCidades'
          Memo.UTF8W = (
            '[RelClientesCidades."Estado_UF_Cidade"]')
        end
        object Memo19: TfrxMemoView
          Left = 37.795300000000000000
          Top = 68.031540000000000000
          Width = 75.590600000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Cidade')
          ParentFont = False
        end
        object Memo20: TfrxMemoView
          Left = 173.858380000000000000
          Top = 68.031540000000000000
          Width = 45.354360000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'UF')
          ParentFont = False
        end
        object RelClientesCidadesCGC_CPF_Cliente: TfrxMemoView
          Left = 298.582870000000000000
          Top = 37.795300000000000000
          Width = 226.771653543307100000
          Height = 18.897650000000000000
          DataField = 'CGC_CPF_Cliente'
          DataSet = frxDBDataset1
          DataSetName = 'RelClientesCidades'
          Memo.UTF8W = (
            '[RelClientesCidades."CGC_CPF_Cliente"]')
        end
        object Memo6: TfrxMemoView
          Left = 298.582870000000000000
          Top = 15.118120000000000000
          Width = 86.929190000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'CPF | CNJP')
          ParentFont = False
        end
        object RelClientesCidadesCEP_Cliente: TfrxMemoView
          Left = 400.630180000000000000
          Top = 90.708720000000000000
          Width = 75.590600000000000000
          Height = 18.897650000000000000
          DataField = 'CEP_Cliente'
          DataSet = frxDBDataset1
          DataSetName = 'RelClientesCidades'
          Memo.UTF8W = (
            '[RelClientesCidades."CEP_Cliente"]')
        end
        object Memo7: TfrxMemoView
          Left = 400.630180000000000000
          Top = 68.031540000000000000
          Width = 75.590600000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'CEP')
          ParentFont = False
        end
        object RelClientesCidadesBairro_Cliente: TfrxMemoView
          Left = 257.008040000000000000
          Top = 90.708720000000000000
          Width = 105.826840000000000000
          Height = 18.897650000000000000
          DataField = 'Bairro_Cliente'
          DataSet = frxDBDataset1
          DataSetName = 'RelClientesCidades'
          Memo.UTF8W = (
            '[RelClientesCidades."Bairro_Cliente"]')
        end
        object Memo8: TfrxMemoView
          Left = 257.008040000000000000
          Top = 68.031540000000000000
          Width = 64.252010000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Bairro')
          ParentFont = False
        end
        object RelClientesCidadesEndereco_Cliente: TfrxMemoView
          Left = 514.016080000000000000
          Top = 90.708720000000000000
          Width = 166.299320000000000000
          Height = 18.897650000000000000
          DataField = 'Endereco_Cliente'
          DataSet = frxDBDataset1
          DataSetName = 'RelClientesCidades'
          Memo.UTF8W = (
            '[RelClientesCidades."Endereco_Cliente"]')
        end
        object Memo9: TfrxMemoView
          Left = 514.016080000000000000
          Top = 68.031540000000000000
          Width = 151.181200000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Endere'#231'o')
          ParentFont = False
        end
        object RelClientesCidadesTelefone_Cliente: TfrxMemoView
          Left = 514.016080000000000000
          Top = 37.795300000000000000
          Width = 166.299320000000000000
          Height = 18.897650000000000000
          DataField = 'Telefone_Cliente'
          DataSet = frxDBDataset1
          DataSetName = 'RelClientesCidades'
          Memo.UTF8W = (
            '[RelClientesCidades."Telefone_Cliente"]')
        end
        object Memo11: TfrxMemoView
          Left = 514.016080000000000000
          Top = 15.118120000000000000
          Width = 94.488250000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Telefone')
          ParentFont = False
        end
        object Shape1: TfrxShapeView
          Top = 11.338590000000000000
          Width = 718.110700000000000000
          Height = 105.826840000000000000
        end
      end
      object GroupHeader1: TfrxGroupHeader
        FillType = ftBrush
        Height = 68.031540000000000000
        Top = 170.078850000000000000
        Width = 718.110700000000000000
        Condition = 'RelClientesCidades."Codigo_Cidade"'
        object Memo12: TfrxMemoView
          Left = 294.803340000000000000
          Top = 11.338590000000000000
          Width = 196.535560000000000000
          Height = 30.236240000000000000
          DataField = 'Nome_Cidade'
          DataSet = frxDBDataset1
          DataSetName = 'RelClientesCidades'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -24
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            '[RelClientesCidades."Nome_Cidade"]')
          ParentFont = False
        end
      end
      object Memo16: TfrxMemoView
        Left = 45.354360000000000000
        Top = 570.709030000000000000
        Width = 52.913420000000000000
        Height = 18.897650000000000000
        Visible = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clRed
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        Memo.UTF8W = (
          'C'#243'digo')
        ParentFont = False
      end
      object Memo5: TfrxMemoView
        Left = 45.354360000000000000
        Top = 551.811380000000000000
        Width = 52.913420000000000000
        Height = 18.897650000000000000
        Visible = False
        DataField = 'Codigo_Cidade'
        DataSet = frxDBDataset1
        DataSetName = 'RelClientesCidades'
        Memo.UTF8W = (
          '[RelClientesCidades."Codigo_Cidade"]')
      end
      object Memo2: TfrxMemoView
        Left = 26.456710000000000000
        Top = 612.283860000000000000
        Width = 56.692950000000000000
        Height = 18.897650000000000000
        Visible = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clRed
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        Memo.UTF8W = (
          'C'#243'digo')
        ParentFont = False
      end
      object Memo1: TfrxMemoView
        Left = 37.795300000000000000
        Top = 665.197280000000000000
        Width = 56.692950000000000000
        Height = 18.897650000000000000
        Visible = False
        DataField = 'Codigo_Cliente'
        DataSet = frxDBDataset1
        DataSetName = 'RelClientesCidades'
        Memo.UTF8W = (
          '[RelClientesCidades."Codigo_Cliente"]')
      end
      object Memo10: TfrxMemoView
        Left = 158.740260000000000000
        Top = 653.858690000000000000
        Width = 52.913420000000000000
        Height = 26.456710000000000000
        Visible = False
        DataField = 'Codigo_Cidade'
        DataSet = frxDBDataset1
        DataSetName = 'RelClientesCidades'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -24
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        Memo.UTF8W = (
          '[RelClientesCidades."Codigo_Cidade"]')
        ParentFont = False
      end
      object Memo13: TfrxMemoView
        Left = 298.582870000000000000
        Top = 642.520100000000000000
        Width = 64.252010000000000000
        Height = 30.236240000000000000
        Visible = False
        DataField = 'Estado_UF_Cidade'
        DataSet = frxDBDataset1
        DataSetName = 'RelClientesCidades'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -24
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        Memo.UTF8W = (
          '[RelClientesCidades."Estado_UF_Cidade"]')
        ParentFont = False
      end
    end
  end
end
